...
var quirkRegistry = []Quirk{
	...
	{ID: 52, Provider: ProviderMoltis, Versions: "observed 20260902.01", Event: KindToolPost,
		Behavior:   "AfterToolCall carries neither a tool-call id nor the original arguments, so a post event cannot be correlated exactly with its pre event from the payload",
		Mitigation: "Tool.Synthesized is true and Raw preserves the native result; consumers must not treat the generated post id as cross-event correlation evidence",
		Reference:  "moltis crates/common/src/hooks.rs"},
	{ID: 53, Provider: ProviderMoltis, Versions: "observed 20260902.01", Event: KindStop,
		...
		Mitigation: "decode and install support remain forward-compatible, but consumers must treat these events as unavailable until a real runtime probe observes them",
		Reference:  "moltis HookEvent enum and repository-wide dispatch-site audit at 77b0d0ac"},
